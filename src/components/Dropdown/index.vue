<template>
  <div
    :class="{ dropdown: true, 'dropdown-isShow': state.isShow }"
...
defineExpose({
  showDropdown(option) {
    ...
    state.isShow = true;
    nextTick(() => {
      // 这里必须等待dom更新完成后再执行，否则获取的dropHeight为0
      ...
      state.top =
        top + dropdownHeight > canvasHeight ? top - dropdownHeight : top; // 判断实现dropMenu是否会超出画布，并做出相应处理
      state.left = left;
    });
  },
});
...
.dropdown {
  display: none;
  position: absolute;
  box-shadow: 0.0429rem 0.0429rem 0.1143rem rgba($color: #000000, $alpha: 0.1);
  background-color: #fff;
  border-radius: 0.0714rem;
  border: #adb5bd 0.0143rem solid;
  width: 2.8571rem;
  padding-bottom: 0.0714rem;
  overflow: hidden;
